...
async def start(bot, message):
    await message.reply(
        ...
        "I'm paisakamalo convert bot. Just send me link and get short link")

@bot.on_message(filters.command('help') & filters.private)
async def start(bot, message):
    await message.reply(
        f"**Hello, {message.chat.first_name}!**\n\n"
        "**If you send post which had dropLinks, texts & images... Than I'll convert & replace all droplinks with your droplinks \nMessage me @jack_sparow119 For more help-**")

@bot.on_message(filters.command('support') & filters.private)
async def start(bot, message):
    await message.reply(
        f"**Hey, {message.chat.first_name}!**\n\n"
        "**please contact me on @jack_sparow119 for any help**")
